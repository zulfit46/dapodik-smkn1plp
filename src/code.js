...
const OFFICIAL_GTK_HEADERS = [
  ...
  "Lintang", "Bujur", "NUKS", "status_login"
];
  ...
  try {
    ...
    if (targetType === "naikpangkat" || targetType === "pangkat") {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if ((targetNip && rNip === targetNip && targetNoSk && rNoSk === targetNoSk) || (targetNoSk && targetNoSk.length > 3 && rNoSk === targetNoSk)) {
          foundRow = i + 1;
          break;
        }
      }
    ...
    if (targetType === "gtk") {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if ((targetNip && rNip === targetNip) || (targetNuptk && rNuptk === targetNuptk)) {
          foundRow = i + 1;
          break;
        }
      }
      ...
      const rowValues = headers.map(h => {
        ...
        if (hClean.includes("tgl") || hClean.includes("tanggal") || hClean.includes("tmt")) {
          return formatToDDMMYYYY(gItem[h] || gItem[hClean] || "");
        }
        if (gItem[h] !== undefined) return String(gItem[h]);
        if (gItem[hClean] !== undefined) return String(gItem[hClean]);
        return "";
      });
