...
import { formatDisplayDate, parseToYYYYMMDD } from '../utils/dateUtils';
...
interface GTKPangkatViewProps {
  gtkList?: GTKData[];
  appConfig?: AppConfig;
  currentUser?: GTKData | null;
}

export const GTKPangkatView: React.FC<GTKPangkatViewProps> = ({ 
  ...
  // State for Riwayat Pangkat data with safe persistence and validation against student data leakage
  const [riwayatList, setRiwayatList] = useState<RiwayatPangkat[]>(() => {
    const cached = safeGetItem<any[]>(STORAGE_KEY, []);
    if (Array.isArray(cached) && cached.length > 0) {
      // Validate that it contains real pangkat records, not student records
      const isStudentData = cached.some(item => item && (item.nipd || item.rombel || item.nisn || item.nama_ayah));
      const hasPangkatFields = cached.some(item => item && (item.gol || item.noSk || item.tmt));
      if (!isStudentData && hasPangkatFields) {
        return cached as RiwayatPangkat[];
      }
    }
    return INITIAL_RIWAYAT_PANGKAT;
  });
  ...
  // A GTK can have multiple historical promotion entries (riwayat pangkat) across their career.
  // We only merge identical duplicate rows (same NIP + same SK + same Golongan/TMT) caused by previous double-save bugs.
  const normalizeAndDeduplicatePangkat = useCallback((items: any[]): RiwayatPangkat[] => {
    const uniqueMap = new Map<string, RiwayatPangkat>();
    items.forEach((item, idx) => {
      const noSk = (item.noSk || item.no_sk || item.nosk || '').trim();
      const nip = (item.nip || '').trim();
      const gol = (item.gol || item.golongan || '').trim();
      const tmt = (item.tmt || '').trim();
      const isPlaceholderSk = !noSk || noSk === '-' || noSk === '0';
      const key = nip
        ? `${nip}_${isPlaceholderSk ? 'nosk' : noSk}_${gol || 'nogol'}_${tmt || (item.timestamp || item.id || `row-${idx}`)}`
        : (item.id || `row-${idx}`);

      const normalized: RiwayatPangkat = {
        id: item.id || `PANGKAT-${nip ? nip + '-' : ''}${idx + 1}`,
        no: item.no || idx + 1,
        nip: nip,
        nama: (item.nama || '').trim(),
        gol: gol,
      ...
      if (uniqueMap.has(key)) {
        ...
        const existingScore = (existing.tglSk ? 1 : 0) + (existing.tmt ? 1 : 0);
        const newScore = (normalized.tglSk ? 1 : 0) + (normalized.tmt ? 1 : 0);
  ...
  const loadPangkatData = useCallback(async (force = false) => {
    setIsLoading(true);
    try {
      // 1. Try Backend API
      const res = await fetch(`/api/pangkat${force ? '?force=true' : ''}`, { signal: AbortSignal.timeout(8000) });
      if (res.ok) {
        const json = await res.json();
        if (json && json.status === 'success' && Array.isArray(json.data) && json.data.length > 0) {
          setRiwayatList(normalizeAndDeduplicatePangkat(json.data));
          setIsLoading(false);
          return;
        }
      }
    } catch (err) {
      console.warn('Backend /api/pangkat fetch error, trying client direct sync...', err);
    }

    // 2. Try Client Direct GViz/WebApp
    try {
      const directData = await fetchPangkatDirectly(appConfig);
      if (directData && directData.length > 0) {
        setRiwayatList(normalizeAndDeduplicatePangkat(directData));
      }
    } catch (err) {
      console.warn('Direct Pangkat fetch error:', err);
    } finally {
      setIsLoading(false);
    }
  }, [appConfig, normalizeAndDeduplicatePangkat]);
  ...
  // If role is 'user', filter riwayat kepangkatan strictly to records matching the user's NIP
  ...
  // Form Fields: No, nip, nama, gol, no_sk, tgl_sk, tmt, masa_kerja_thn, masa_kerja_bln, timestamp, status
  ...
  const handleSelectGTK = (selectedVal: string) => {
    ...
    if (gtk) {
      ...
      let shortGol = (gtk.pangkatGolongan || (isPppk ? 'IX' : 'III/a')).trim();
      const matchParen = shortGol.match(/\(([^)]+)\)/);
      if (matchParen && matchParen[1]) {
        shortGol = matchParen[1].trim();
      } else if (shortGol.includes(' - ')) {
        shortGol = shortGol.split(' - ')[0].trim();
      }
  ...
  // Start Edit
  const handleStartEdit = (item: RiwayatPangkat) => {
    if (isUser && !isOwnerOfRecord(item.nip, item.nama, currentUser)) {
      setFeedbackMsg({
        type: 'error',
        text: 'Anda hanya dapat mengedit data riwayat kepangkatan milik akun Anda sendiri.'
      });
      return;
    }
    ...
    const rawTmt = item.tmt || '';
    const rawMasaThn = item.masaKerjaThn !== undefined ? item.masaKerjaThn : ((item as any).masa_kerja_thn || 0);
    const rawMasaBln = item.masaKerjaBln !== undefined ? item.masaKerjaBln : ((item as any).masa_kerja_bln || 0);

    let editGol = (item.gol || (item as any).golongan || '').trim();
    if (editGol.includes(' - ')) {
      editGol = editGol.split(' - ')[0].trim();
    }
    const matchParen = editGol.match(/\(([^)]+)\)/);
    if (matchParen && matchParen[1]) {
      editGol = matchParen[1].trim();
    }
    ...
    const isPppk = (matchedGtk && (matchedGtk.statusKepegawaian || '').toUpperCase().includes('PPPK')) ||
      (DAFTAR_GOLONGAN_PPPK.includes(editGol) && !editGol.includes('/'));

    setFormData({
      no: item.no ? String(item.no) : '',
      nip: item.nip || '',
      nama: item.nama || '',
      gol: editGol || (isPppk ? 'IX' : 'III/a'),
  ...
  const handleJumpToRecord = (recordId: string) => {
    ...
    if (targetIdx >= 0) {
      ...
      setTimeout(() => {
        const el = document.getElementById(`row-pangkat-${recordId}`);
        if (el) {
  ...
  const handleDelete = (item: RiwayatPangkat) => {
    if (isUser && !isOwnerOfRecord(item.nip, item.nama, currentUser)) {
      setFeedbackMsg({
        type: 'error',
        text: 'Anda hanya dapat menghapus data riwayat kepangkatan milik akun Anda sendiri.'
      });
      return;
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (appConfig.webAppUrl) {
        deletePangkatDirectly(appConfig.webAppUrl, {
      ...
      setFeedbackMsg({
        type: 'success',
        text: `Riwayat kepangkatan untuk ${itemToDelete.nama} (SK: ${itemToDelete.noSk || '-'}) berhasil dihapus.`
      });
    } catch (err: any) {
      setFeedbackMsg({
        type: 'error',
  ...
  const duplicateDetectedInForm = useMemo(() => {
    ...
    return riwayatList.find(item => {
      ...
      const itemTmt = formatToDDMMYYYY(item.tmt || (item as any).TMT || (item as any).tmt_pangkat);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.noSk.trim()) {
      setFeedbackMsg({ type: 'error', text: 'Nomor SK wajib diisi!' });
      return;
    }
    if (!formData.tglSk || !formData.tmt) {
      setFeedbackMsg({ type: 'error', text: 'Tanggal SK dan TMT wajib diisi!' });
      return;
    }
    ...
    if (duplicateEntry) {
      setDuplicateModalData({
        ...
        noSk: trimmedNoSk,
        tmt: formattedTmt,
        extraInfo: `Golongan: ${duplicateEntry.gol || formData.gol} | Masa Kerja: ${duplicateEntry.masaKerjaThn || 0} Thn ${duplicateEntry.masaKerjaBln || 0} Bln | Status: ${duplicateEntry.status || 'Aktif'}`
      });
      setFeedbackMsg({
        type: 'error',
        text: `Data riwayat sudah ada! Riwayat Kepangkatan untuk NIP ${trimmedNip} dengan Nomor SK "${trimmedNoSk}" dan TMT "${formattedTmt}" sudah terdaftar sebelumnya. Tidak dapat menyimpan data duplikat.`
      });
    ...
    const savedDetails: SavedDetailItem[] = [
      ...
      { label: 'Pangkat / Golongan', value: formData.gol },
      { label: 'Nomor SK', value: formData.noSk.trim() },
      { label: 'Tanggal SK', value: formattedTglSk },
      { label: 'TMT Pangkat', value: formattedTmt },
    ...
    try {
      if (editingId) {
        // Update
        const updatedItem: RiwayatPangkat = {
          id: editingId,
          no: formData.no || '',
        ...
        setSuccessModalData({
          isOpen: true,
          title: 'Riwayat Pangkat Berhasil Diperbarui!',
          subtitle: `Data riwayat kepangkatan untuk ${formData.nama} telah tersimpan dan disinkronkan ke Spreadsheet.`,
          actionType: 'update',
          dataDetails: savedDetails,
          timestamp: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
        });

        // 2. Sinkronisasi ke backend & Google Apps Script di background
        if (appConfig.webAppUrl) {
          savePangkatDirectly(appConfig.webAppUrl, updatedItem, true).catch(err => {
            console.warn('Background sync to Google Apps Script failed:', err);
          });
          fetch(`/api/pangkat/${encodeURIComponent(editingId)}?forward=false`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...updatedItem, skipSheetsForward: true })
          }).catch(e => console.warn('Backend update cache error:', e));
        } else {
          fetch(`/api/pangkat/${encodeURIComponent(editingId)}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        // Create new
        const newEntryId = `PANGKAT-${Date.now()}`;
        const newEntry: RiwayatPangkat = {
          id: newEntryId,
        ...
        setTimeout(() => {
          const el = document.getElementById(`row-pangkat-${newEntryId}`);
          if (el) {
            el.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
        }, 300);
        ...
        setSuccessModalData({
          isOpen: true,
          title: 'Riwayat Pangkat Berhasil Ditambahkan!',
          subtitle: `Data SK kenaikan pangkat untuk ${formData.nama} berhasil tersimpan sebagai teks.`,
          actionType: 'create',
          dataDetails: savedDetails,
          timestamp: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
        });
        ...
        if (appConfig.webAppUrl) {
          savePangkatDirectly(appConfig.webAppUrl, newEntry, false).catch(err => {
            console.warn('Background sync to Google Apps Script failed:', err);
          });
          fetch('/api/pangkat?forward=false', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleSyncGoogleSheets = async () => {
    ...
    try {
      ...
      if (appConfig.webAppUrl) {
        promises.push(syncAllPangkatDirectly(appConfig.webAppUrl, riwayatList));
      }
      promises.push(
        fetch('/api/pangkat/sync', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (success || appConfig.webAppUrl) {
        setSuccessModalData({
          isOpen: true,
          title: 'Sinkronisasi Seluruh Riwayat Pangkat Berhasil!',
          subtitle: `Sebanyak ${riwayatList.length} data riwayat kepangkatan telah berhasil disinkronkan ke Spreadsheet.`,
          actionType: 'syncAll',
          dataDetails: [
            { label: 'Jumlah Baris Disinkronkan', value: `${riwayatList.length} data riwayat kepangkatan` },
  ...
  const handleRestoreDefault = () => {
    if (window.confirm('Reset riwayat kepangkatan ke data awal bawaan sistem? Data yang belum tersimpan ke backup akan terganti.')) {
      setRiwayatList(INITIAL_RIWAYAT_PANGKAT);
      safeSetItem(STORAGE_KEY, INITIAL_RIWAYAT_PANGKAT);
      setFeedbackMsg({ type: 'success', text: 'Data riwayat kepangkatan berhasil dikembalikan ke data awal.' });
  ...
  // Export Excel matching exact Sheet "naikpangkat" headers: No, nip, nama, gol, no_sk, tgl_sk, tmt, masa_kerja_thn, masa_kerja_bln, timestamp, status
  const handleExportExcel = () => {
    const data = filteredRiwayat.map((r, idx) => ({
      ...
      'tgl_sk': r.tglSk || '-',
      'tmt': r.tmt || '-',
      'masa_kerja_thn': r.masaKerjaThn || 0,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'naikpangkat');

    const dateStr = new Date().toISOString().split('T')[0];
    XLSX.writeFile(workbook, `naikpangkat_${dateStr}.xlsx`);
  };
  ...
  const handlePrintPDF = () => {
    ...
    doc.setFont('times', 'bold');
    doc.setFontSize(11);
    doc.text('DAFTAR RIWAYAT KEPANGKATAN GURU & TENAGA KEPENDIDIKAN', pageWidth / 2, 39, { align: 'center' });
    doc.setFont('times', 'normal');
    doc.setFontSize(8.5);
    ...
    autoTable(doc, {
      startY: 47,
      head: [['No', 'NIP', 'Nama Lengkap', 'Golongan', 'No. SK', 'Tgl SK', 'TMT', 'Masa Kerja', 'Status']],
      body: tableBody,
      theme: 'grid',
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      {isUser && currentUser && (
        <div className="bg-indigo-50 border border-indigo-200/90 rounded-2xl p-4 sm:p-5 flex items-center justify-between gap-4 text-indigo-950 shadow-xs">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h2 className="text-sm sm:text-base font-bold text-indigo-950 leading-tight">
                {currentUser.nama}
              </h2>
      ...
      {/* FORM INPUT RIWAYAT KEPANGKATAN */}
      <div className="bg-white border border-slate-300 rounded-xl shadow-2xs overflow-hidden">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm sm:text-base font-bold tracking-tight">
                  {editingId ? 'Edit Data Riwayat Kepangkatan GTK' : 'Form Input Riwayat Kepangkatan GTK'}
                </h2>
              </div>
              <p className="text-xs text-indigo-200">
                {editingId ? 'Perbarui data riwayat kepangkatan yang dipilih' : 'Tambahkan data riwayat kenaikan pangkat & golongan baru'}
              </p>
            </div>
          </div>
        ...
        {isFormOpen && (
          <form onSubmit={handleSubmit} className="p-5 sm:p-6 bg-slate-50/70 border-b border-slate-200 space-y-5 animate-in slide-from-top-2 duration-200">
            ...
            <div className="bg-indigo-50/80 p-3.5 rounded-xl border border-indigo-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
              ...
              <select
                ...
                className="px-3 py-1.5 rounded-lg border border-indigo-200 bg-white font-medium text-slate-800 focus:outline-hidden focus:ring-2 focus:ring-indigo-500 cursor-pointer text-xs"
              >
                <option value="">-- Pilih GTK untuk Isi Otomatis --</option>
                {effectiveGtkList.map((g, idx) => (
                  <option key={`opt-pangkat-${g.id || g.nip || g.nama || 'gtk'}-${idx}`} value={g.nip || g.nama}>
                    {g.nama}
                  </option>
                ))}
              </select>
            ...
            {/* Grid Form Inputs matching sheet header: nip, nama, gol, no_sk, tgl_sk, tmt, masa_kerja_thn, masa_kerja_bln, status */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
              {/* 1. NIP */}
              <div>
                <label className="block font-semibold text-slate-700 mb-1.5">
                  NIP <span className="text-rose-500">*</span>
                  {isUser && <span className="text-[10px] text-indigo-600 ml-1 font-normal">(Terkunci Akun)</span>}
                </label>
                <input
                  type="text"
                  required
                  ...
                  className={`w-full px-3 py-2 rounded-lg border font-mono text-slate-800 focus:outline-hidden focus:ring-2 focus:ring-indigo-500 ${
              ...
              <div className="lg:col-span-2">
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 rounded-lg border font-semibold text-slate-800 focus:outline-hidden focus:ring-2 focus:ring-indigo-500 ${
                    isUser ? 'bg-slate-100 border-slate-200 cursor-not-allowed text-slate-600' : 'bg-white border-slate-300'
                  }`}
                />
              </div>
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1.5">
                  Golongan / Pangkat <span className="text-rose-500">*</span>
                </label>
                <select
                  value={formData.gol}
              ...
              <div className="lg:col-span-2">
                <label className="block font-semibold text-slate-700 mb-1.5">
                  Nomor SK Pangkat (no_sk) <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  required
                  ...
                  placeholder="Contoh: 823.4/125/BKPSDM/2022"
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1.5">
                  TMT Pangkat (tmt) <span className="text-rose-500">*</span>
                </label>
                <input
                  type="date"
                  required
              ...
              {/* 7. Masa Kerja Tahun (masa_kerja_thn) */}
              <div>
                <label className="block font-semibold text-slate-700 mb-1.5">
                  Masa Kerja Tahun (masa_kerja_thn) <span className="text-rose-500">*</span>
                </label>
              ...
              <div className="lg:col-span-2">
                ...
                <select
                  id="input-status-berkas-pangkat"
                  disabled={isUser}
                ...
                {isUser ? (
                  ...
                  <p className="text-[11px] text-slate-500 mt-1">
                    Status berkas kenaikan pangkat: <strong>Proses</strong> (pengajuan), <strong>Selesai</strong> (disetujui), atau <strong>Aktif</strong>.
                  </p>
                )}
              </div>
            </div>
            ...
            {duplicateDetectedInForm && (
              <div 
                id="inline-duplicate-warning-pangkat"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-xs space-y-1">
                    ...
                    <p className="text-amber-800 leading-relaxed">
                      Data riwayat kepangkatan untuk <strong>{duplicateDetectedInForm.nama}</strong> (NIP: {duplicateDetectedInForm.nip}) dengan Nomor SK <strong>"{formData.noSk.trim()}"</strong> dan TMT <strong>"{formatToDDMMYYYY(formData.tmt)}"</strong> sudah pernah tersimpan di sistem (Status: <strong>{duplicateDetectedInForm.status || 'Aktif'}</strong>).
                    </p>
                  </div>
                </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-200">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl shadow-xs transition-colors cursor-pointer text-xs disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              >
                {isSaving ? (
                  ...
                  <>
                    ...
                    <span>{editingId ? 'Simpan Perubahan' : 'Simpan Riwayat Pangkat'}</span>
                  </>
                )}
              </button>
            </div>
      ...
      {/* TABEL DATA RIWAYAT KEPANGKATAN - Clean Grid Style (Matching Verval PD) */}
      <div className="bg-white border border-slate-300 rounded-lg shadow-2xs overflow-hidden w-full">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 space-y-4 bg-slate-50/50">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            {!isUser && (
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  placeholder="Cari berdasarkan NIP, Nama, Golongan, No SK..."
            ...
            <div className={`flex flex-wrap items-center gap-2 ${isUser ? 'ml-auto' : ''}`}>
              ...
              <button
                ...
                title="Muat ulang data dari Google Sheets sheet 'naikpangkat'"
              >
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-200/80 text-xs">
            <span className="text-slate-500">
              Menampilkan <strong>{filteredRiwayat.length}</strong> dari <strong>{effectiveRiwayatList.length}</strong> riwayat kepangkatan
            </span>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-100 text-slate-800 font-semibold text-xs border-b border-slate-300">
                ...
                <th className="py-2.5 px-3 min-w-[120px] text-center border-r border-slate-300">Golongan</th>
                <th className="py-2.5 px-3 min-w-[170px] border-r border-slate-300">Nomor SK (no_sk)</th>
                <th className="py-2.5 px-3 min-w-[110px] text-center border-r border-slate-300">Tgl SK</th>
                <th className="py-2.5 px-3 min-w-[110px] text-center border-r border-slate-300">TMT</th>
                <th className="py-2.5 px-3 min-w-[130px] text-center border-r border-slate-300">Masa Kerja</th>
                <th className="py-2.5 px-3 min-w-[100px] text-center border-r border-slate-300">Status</th>
              ...
              {paginatedRiwayat.length === 0 ? (
                <tr>
                  <td colSpan={10} className="py-12 text-center text-slate-500 italic">
                    ...
                    <p className="font-semibold text-slate-600">Belum ada data riwayat kepangkatan.</p>
                    <p className="text-[11px] text-slate-400 mt-1">Klik tombol "Tambah Pangkat" atau "Muat Ulang" untuk mengambil data.</p>
                  </td>
                </tr>
              ) : (
                paginatedRiwayat.map((item, idx) => {
                  ...
                  const isHighlighted = highlightedRowId === item.id;
                    return (
                    <tr 
                      id={`row-pangkat-${item.id || idx}`}
                      key={`pangkat-row-${item.id || item.nip || 'id'}-${idx}`}
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 text-center font-medium text-slate-800 border-r border-slate-200 text-xs">
                        {formatDisplayDate(item.tmt)}
                      </td>

                      <td className="py-2.5 px-3 text-center font-medium text-slate-800 border-r border-slate-200">
                        {item.masaKerjaThn !== undefined ? item.masaKerjaThn : ((item as any).masa_kerja_thn || 0)} Thn {item.masaKerjaBln !== undefined ? item.masaKerjaBln : ((item as any).masa_kerja_bln || 0)} Bln
                      </td>
                      ...
                      <td className="py-2.5 px-3 text-center sticky right-0 bg-white shadow-xs border-l border-slate-200">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            ...
                            title="Edit Riwayat Kepangkatan"
                          >
                            <Edit className="w-4 h-4" />
                          </button>
                          <button
                            id={`btn-delete-pangkat-${item.id || idx}`}
                            type="button"
        ...
        <div className="p-3 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs text-slate-600">
          <div className="flex items-center gap-2">
            ...
            <span>
              dari <strong className="text-slate-900">{totalItems}</strong> data Riwayat Pangkat
            </span>
          </div>
      ...
      <DeleteConfirmationModal
        ...
        itemDetails={deleteModalState.item ? [
          ...
          { label: 'Golongan/Pangkat', value: deleteModalState.item.gol || '-' },
          { label: 'Nomor SK Pangkat', value: deleteModalState.item.noSk || '-' },
