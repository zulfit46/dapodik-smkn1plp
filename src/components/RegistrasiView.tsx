...
export const RegistrasiView: React.FC<RegistrasiViewProps> = ({ students }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter(s => 
      ...
      (s.sekolahAsal || '').toLowerCase().includes(q)
    );
  }, [students, search]);
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
            <FileText className="w-5 h-5 text-indigo-600" /> Registrasi Peserta Didik
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Informasi status pendaftaran, sekolah asal, dan tanggal resmi masuk sekolah.
          </p>
        </div>
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            placeholder="Cari nama, NIPD, kelas..."
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-100 text-slate-800 font-semibold text-xs border-b border-slate-300">
                ...
                <th className="py-2.5 px-3 border-r border-slate-300">Tanggal Masuk</th>
                <th className="py-2.5 px-4 border-r border-slate-300">Sekolah Asal</th>
                <th className="py-2.5 px-3">No. HP Siswa</th>
              </tr>
            </thead>
                ...
                paginatedStudents.map((student, idx) => {
                  ...
                  return (
                    <tr key={student.id ? `${student.id}-${idx}` : `stu-${idx}`} className="hover:bg-slate-50 transition-colors border-b border-slate-200">
                      ...
                      <td className="py-2.5 px-3 text-slate-800 border-r border-slate-200">{student.tanggalMasuk || '2022-07-11'}</td>
                      <td className="py-2.5 px-4 text-slate-800 border-r border-slate-200">{student.sekolahAsal || 'SMP Negeri 1'}</td>
                      <td className="py-2.5 px-3 text-slate-800">{student.noHp || '-'}</td>
                    </tr>
                  );
                })
              )}
